...
export const fetchExams = createAsyncThunk('exam/fetchExams', async (_,{rejectWithValue})=>{
  try{const r=await axios.get(API_URL); return r.data;}catch(e){return rejectWithValue(e.response?.data?.message||'Failed');}
});
export const fetchMyExams = createAsyncThunk('exam/fetchMyExams', async (_,{rejectWithValue})=>{
  try{const r=await axios.get(`${API_URL}/my`); return r.data;}catch(e){
    try{const r2=await axios.get(API_URL); return r2.data;}catch{return rejectWithValue('Failed');}
  }
});
export const fetchExamById = createAsyncThunk('exam/fetchExamById', async (id,{rejectWithValue})=>{
  try{const r=await axios.get(`${API_URL}/${id}`); return r.data;}catch(e){return rejectWithValue(e.response?.data?.message||'Failed');}
});
// ResultDashboard এর জন্য এটাই missing ছিল
export const fetchExamResults = createAsyncThunk('exam/fetchExamResults', async (examId,{rejectWithValue})=>{
  try{const r=await axios.get(`${API_URL}/${examId}/results`); return r.data;}
  catch(e){return rejectWithValue(e.response?.data?.message||'Failed to fetch results');}
});

// CREATE / UPLOAD
export const createExam = createAsyncThunk('exam/createExam', async (data,{rejectWithValue})=>{
  try{const r=await axios.post(API_URL,data); return r.data;}catch(e){return rejectWithValue(e.response?.data?.message||'Failed');}
});
export const uploadExamPdf = createAsyncThunk('exam/uploadExamPdf', async (formDataArg,{rejectWithValue})=>{
  try{
    const fd = formDataArg instanceof FormData? formDataArg : formDataArg.formData || formDataArg;
    const r=await axios.post(`${API_URL}/upload`, fd, {headers:{'Content-Type':'multipart/form-data'}});
    return r.data;
  }catch(e){return rejectWithValue(e.response?.data?.message||'PDF upload failed');}
});
...
export const updateExam = createAsyncThunk('exam/updateExam', async ({id,examData},{rejectWithValue})=>{
  try{const r=await axios.put(`${API_URL}/${id}`,examData); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const deleteExam = createAsyncThunk('exam/deleteExam', async (id,{rejectWithValue})=>{
  try{await axios.delete(`${API_URL}/${id}`); return id;}catch(e){return rejectWithValue('Failed');}
});
export const updateExamSettings = createAsyncThunk('exam/updateExamSettings', async ({id,payload},{rejectWithValue})=>{
  try{const r=await axios.put(`${API_URL}/${id}`,payload); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const publishExam = createAsyncThunk('exam/publishExam', async (id,{rejectWithValue})=>{
  try{const r=await axios.post(`${API_URL}/${id}/publish`); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const setResourceLink = createAsyncThunk('exam/setResourceLink', async ({id,link},{rejectWithValue})=>{
  try{const r=await axios.put(`${API_URL}/${id}/resource`,{link}); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const uploadResourcePdf = createAsyncThunk('exam/uploadResourcePdf', async ({id,formData},{rejectWithValue})=>{
  try{const r=await axios.post(`${API_URL}/${id}/resource/pdf`,formData,{headers:{'Content-Type':'multipart/form-data'}}); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const removeResource = createAsyncThunk('exam/removeResource', async (id,{rejectWithValue})=>{
  try{const r=await axios.delete(`${API_URL}/${id}/resource`); return r.data;}catch(e){return rejectWithValue('Failed');}
});

// QUESTIONS
export const addQuestion = createAsyncThunk('exam/addQuestion', async ({examId,questionData},{rejectWithValue})=>{
  try{const r=await axios.post(`${API_URL}/${examId}/questions`,questionData); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const updateQuestion = createAsyncThunk('exam/updateQuestion', async ({questionId,payload},{rejectWithValue})=>{
  try{const r=await axios.put(`${API_URL}/questions/${questionId}`,payload); return r.data;}catch(e){return rejectWithValue('Failed');}
});
export const deleteQuestion = createAsyncThunk('exam/deleteQuestion', async (arg,{rejectWithValue})=>{
  try{const qId=typeof arg==='string'?arg:arg.questionId||arg._id; await axios.delete(`${API_URL}/questions/${qId}`); return qId;}catch(e){return rejectWithValue('Failed');}
});

const initialState = { exams:[], currentExam:null, questions:[], results:[], shareLink:null, loading:false, error:null };
...
const examSlice = createSlice({
  ...
  extraReducers:(b)=>{
   ...
   .addCase(uploadExamPdf.fulfilled,(s,a)=>{const ex=a.payload.exam||a.payload; if(ex?._id) s.exams.push(ex);})
